...
public:
  Dof<ShapeFct>(): mesh_p(0), node_p(0), offset(0) {};
  // Copy-Constructor
  Dof<ShapeFct>(const Dof<ShapeFct>& dof): mesh_p(dof.mesh_p), 
                                     node_p(dof.node_p), 
                                     elt_to_dof(dof.elt_to_dof),
                                     dof_to_elt(dof.dof_to_elt),
                                     nb_elt(dof.nb_elt),
                                     nb_dof(dof.nb_dof),
                                     offset(dof.offset){};
  //
  ...
  mesh_p(&m), node_p(&GeometryOf(m)), nb_elt(NbElt(m)), offset(0) {
    ...
    std::vector<int> num(nb_node,-1);               // Initialize with -1
    for(int j=0; j<NbElt(mesh); j++){               // For each element  
      array<Trait::dim+1,int> I = Num(mesh[j]);     // Element nodes
      for(int k=0; k<Trait::dim+1; k++){            // For each node
	if(num[I[k]]==-1){                                // If it is a new node, 
	  num[I[k]]=nb_dof;                               // Number it nb_dof
	  dof_to_elt.push_back(std::vector<N2>());        // Add a new N2 vector 
	  nb_dof++;                                       // Increase the counter
	}
