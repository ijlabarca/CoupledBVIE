...
class Interpolator< BasisFct<P0,D> >{
 ...
 public:
  Interpolator< BasisFct<P0,D> >(const Mesh<D>& m): mesh(m), normal(NormalTo(m)) {};
  ...
  template <typename fct_t>
    ReturnType operator()(const fct_t& fx,const fct_t& fy,const fct_t& fz, const int& j){
    ny = normal[j];
    ReturnType res; res[0] = fx( Ctr(mesh[j]) ) * ny[0] +
                             fy( Ctr(mesh[j]) ) * ny[1] +
                             fz( Ctr(mesh[j]) ) * ny[2];
    return res; }
