...
namespace bemtool {
  ...
  template<typename VIOpType>
  class SubVIOp{

    typedef typename VIOpType::KernelTypeTrait  KernelTrait;
    typedef typename KernelTrait::ShapeFctX     PhiX;
    typedef typename KernelTrait::ShapeFctY     PhiY;
    typedef typename KernelTrait::MatType       MatType;
    typedef Dof<PhiX>                           DofX;
  ...
  public:
    SubVIOp<VIOpType>(
		      const DofX&     dofx0,
		      const DofY&     dofy0, const double& kappa): viop(MeshOf(dofx0),MeshOf(dofy0),kappa), dofx(dofx0), dofy(dofy0), Mloc(MeshOf(dofx0)) {}
