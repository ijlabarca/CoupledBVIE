...
/*======== Ignacio ============
  VOLUME OPERATOR HELMHOLTZ 3D
  =============================*/


template <typename PhiX, typename PhiY>
class VIOpKernel<HE,VL_OP,3,PhiX,PhiY>{

public:
  typedef VIOpKernelTraits<HE,VL_OP,3,PhiX,PhiY> Trait;
...
public:
  VIOpKernel<HE,VL_OP,3,PhiX,PhiY>(const typename Trait::MeshX& mx,
				   const typename Trait::MeshY& my,
				   const Real& k):
  meshx(mx), phix(mx), meshy(my), phiy(my), kappa(k) {};
...
typedef VIOpKernel<HE,VL_OP,3,P0_3D,P0_3D> HE_VL_3D_P0xP0;
typedef VIOpKernel<HE,VL_OP,3,P1_3D,P1_3D> HE_VL_3D_P1xP1;

typedef VIOpKernel<HE,VL_OP,3,P0_2D,P1_3D> HE_VL_2Dx3D_P0xP1;

typedef VIOpKernel<HE,VL_OP,3,P1_3D,P0_2D> HE_SL_3Dx2D_P1xP0; // Single Layer



/*======== Ignacio ============
  NEUMANN TRACE VOLUME OPERATOR HELMHOLTZ 3D
  =============================*/

template <typename PhiX, typename PhiY>
class VIOpKernel<HE,NVL_OP,3,PhiX,PhiY>{

public:
  typedef VIOpKernelTraits<HE,NVL_OP,3,PhiX,PhiY> Trait;
...
public:
  VIOpKernel<HE,NVL_OP,3,PhiX,PhiY>(const typename Trait::MeshX& mx,
				    const typename Trait::MeshY& my,
				    const Real& k):
  meshx(mx), phix(mx), meshy(my), phiy(my), kappa(k),
    normalx(NormalTo(mx)) {};
  ...
  void Assign(const int& ix, const int& iy){
    //std::cout << "Kernel Assign starts" << std::endl;
    ...
    //std::cout << "Kernel Assign before normal" << std::endl;
    nx    = normalx[ix];
    
    //std::cout << nx << std::endl;
    //std::cout << "Kernel Assign after normal" << std::endl;
  }
...
typedef VIOpKernel<HE,NVL_OP,3,P1_2D,P1_3D> HE_NVL_2Dx3D_P1xP1;
...
template <typename PhiX, typename PhiY>
class VIOpKernel<HE,DVL_OP,3,PhiX,PhiY>{

public:
  typedef BIOpKernelTraits<HE,DVL_OP,3,PhiX,PhiY> Trait;
...
public:
  VIOpKernel<HE,DVL_OP,3,PhiX,PhiY>(const typename Trait::MeshX& mx,
				    const typename Trait::MeshY& my,
				    const Real& k):
  meshx(mx), phix(mx), meshy(my), phiy(my),grad_phiy(my), kappa(k) {};
  ...
  const typename Trait::MatType& operator()(const typename Trait::Rdx& tx,
    ...
    r   = norm2(x_y);
    r3  = r*r*r;
    ker = h*(iu*kappa*r-1.)*exp(iu*kappa*r)/(4*pi*r3); 
    for(int j=0; j<Trait::nb_dof_x; j++){
      for(int k=0; k<Trait::nb_dof_y; k++){
	inter(j,k) = ker*phix(j,tx)*(x_y, grad_phiy(k,ty));
      }
    }
    return inter;
  }
  ...
  const Cplx& operator()(const typename Trait::Rdx& tx,
    ...
    r3  = r*r*r;
    ker = h*(iu*kappa*r-1.)*exp(iu*kappa*r)/(4*pi*r3);
    return ker *= phix(kx,tx)*(x_y, grad_phiy(ky,ty));
  }


};

typedef VIOpKernel<HE,DVL_OP,3,P1_3D,P1_3D> HE_DVL_3D_P1xP1;
...
/*=============================
  DOUBLE LAYER POTENTIAL HELMHOLTZ EN 3D
  =============================*/

template <typename PhiX, typename PhiY>
class VIOpKernel<HE,DL_OP,3,PhiX,PhiY>{

public:
  typedef VIOpKernelTraits<HE,DL_OP,3,PhiX,PhiY> Trait;
...
public:
  VIOpKernel<HE,DL_OP,3,PhiX,PhiY>(const typename Trait::MeshX& mx,
				   const typename Trait::MeshY& my,
				   const Real& k):
  meshx(mx), phix(mx), meshy(my), phiy(my), kappa(k),
    normaly(NormalTo(my)) {};
...
typedef VIOpKernel<HE,DL_OP,3,P1_3D,P1_2D> HE_DL_3Dx2D_P1xP1;
