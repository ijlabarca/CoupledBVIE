...
template<typename T>
void disp(matrix<T>const& A, int info=2, std::ostream& flux=std::cout, std::size_t m=30, std::size_t n=30);
...
matrix<T> gmres(std::function<matrix<T>(matrix<T>const&)>const& A, matrix<T>const& B,
                ...
                matrix<T>const& X0 = matrix<T>(), int info=0)
{
    // Initialize
    if (info>0)
    {
